...
save = save_file(save, "h", "p", "c", "Gamma", "Vs", "grad_gamma")
hs = u.sub(0)
ps = u.sub(1)
cs = u.sub(2)
Gammas = u.sub(3)
...
def save_h(t_arr, h_circ, h_domain):
    ...
    header = f"Nond time, nond average thickness in centre, nond average thickness for "\
             ...
             + f"Ma = {Ma.value :.6e}, Bq_sh = {Bq_sh.value :.6e}, Bq_d = {Bq_d.value :.6e}"
    save_arr = np.column_stack((t_arr, h_circ, h_domain))
...
if rank == 0:
    save_h(np.array(t_save), np.array(h_circ_l), np.array(h_domain_l))
