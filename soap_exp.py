...
# Meshes with uniform cells for Ldomain = 18.25
# mesh_dir = "/home/nav/navScripts/fenicsx/ferro-thickness/mesh/uniform_meniscus15249/"
# Ldomain = 18.25  # Not sure how to automate this as each processor owns parts of the domain
# mesh_file = "uniform_meniscus.xdmf"
...
Gr = fem.Constant(domain, ScalarType(245))  # Gravity number
...
C = fem.Function(V2)  # Centreline height
...
save = save_file(save, "h", "p", "c", "Gamma", "Vs", "gradp", "grad_gamma")
hs = u.sub(0)
ps = u.sub(1)
cs = u.sub(2)
Gammas = u.sub(3)
...
# Save gradGamma over the domain"
save_grad_gamma = 1
if save_grad_gamma:
    grad_gamma = fem.Function(W0)
    grad_gamma.name = "grad_gamma"
    grad_gamma_expr = fem.Expression(Ma * grad(gamma(Gamma)), W0.element.interpolation_points())
    grad_gamma.interpolate(grad_gamma_expr)
    save["grad_gamma"].write_function(grad_gamma, 0.0)
...
def save_h(t_arr, h_circ, h_arr):
    """ Save average h over the domain """
    h_time_dir = "/home/nav/navScripts/fenicsx/ferro-thickness/paper/EXPSPM/"
    ...
    save_arr = np.column_stack((t_arr, h_circ, h_arr))
...
while (step < nsteps):
    ...
    if step % 1 == 0:
        ...
        grad_gamma.interpolate(grad_gamma_expr)
        save["grad_gamma"].write_function(grad_gamma, t)
        # mforce.interpolate(mforce_expr)
...
if rank == 0:
    ...
    # save_params()
    save_h(np.array(t_t), np.array(h_circ_t), np.array(h_domain_t))
