...
def plot_func(fem_func, func_str, save=0):
...
Bq_s = fem.Constant(domain, ScalarType(1.0e-3))  # Shear Boussinesq number
Psi = fem.Constant(domain, ScalarType(0.0))  # H * epsilon * mu_0 * Hc * Mc / (mu * U)

# Non-dimensional terms relevant to magnetophoresis
alpha = fem.Constant(domain, ScalarType(16.0))  # mu0 * m * H / (kb * T) 
beta = fem.Constant(domain, ScalarType(1.6e-2))  # b * mu0 * m * Hc / (U * L)
Pe_c = fem.Constant(domain, ScalarType(1e-1))  # U * L / Dc
...
H = fem.Function(V)  # Magnitude of magnetic field intensity vector field - nond
B = fem.Function(V2)  # Shape of centreline
...
hn.value = np.tan(30.0 * np.pi / 180)  # partialh / partialn
...
def M(c_t):
    """ Magnetisation as a function of concentration """
...
L1 = p * v1 * dx - epsilon * Gr * (h + B) * v1 * dx \
...
L2 = c * v2 * dx - c_n * v2 * dx \
    - dt * dot(c_mid * Vs_mid - 1/3 * c_mid * h_mid**2 * 
               (grad(p_mid) - Psi * M(c_mid) * grad(H)), grad(v2)) * dx \
    - dt * beta * c_mid * langevin(alpha * H) * dot(grad(H), grad(v2)) * dx \
    + dt / Pe_c * dot(grad(c_mid), grad(v2)) * dx \
    # + dt * Jn_c * v2 * ds(1) # - dt * J * v2 * ds(2)
...
def free_shape(x):
    ...
    xc = Ldomain / 2
    yc = Ldomain / 2
    Bc = R - np.sqrt(R**2 - xc**2 / (epsilon_loc**2))  # B at the centre
    # print(f"{Bc = }")
    zc = Bc - R 
    B = zc + np.sqrt(R**2 - 1 / epsilon_loc**2 * ((x[0] - xc)**2 + (x[1] - yc)**2))
    # When plotting, multiply by epsilon so both the domain and what you are plotting
    # are normalised by L
    # B = epsilon_loc * (zc + np.sqrt(R**2 - 1 / epsilon_loc**2 * ((x[0] - xc)**2 + (x[1] - yc)**2)))
    return B

# B.interpolate(free_shape)
# B.x.array[:] = 0
# plot_func(B, "B", save=0)
# sys.exit(0)

save_curvB = 0
if save_curvB:
    curvB = fem.Function(DG0)
    curvB.name = "centreline_curvature"
    curvB_expr = fem.Expression(div(grad(B)), DG0.element.interpolation_points())
    curvB.interpolate(curvB_expr)
    save_curvB = io.XDMFFile(comm, sol_dir + "curvB.xdmf", "w")
    save_curvB.write_mesh(domain)
    save_curvB.write_function(curvB)
    save_curvB.close()
    print(f"Saved {case_dir}curvB.xdmf")
    sys.exit(0)
...
def import_Hmag():
    """ Import the magnetic field created by magpy """
    Hdir = main_dir + "Hfield/left_meniscus/"  # Include 2 mm meniscus length
    lcap = 2  # Capillary length (mm)
    xpos = np.loadtxt(Hdir + 'xpos.txt')
    ypos = np.loadtxt(Hdir + 'ypos.txt')
    # print(f"{xpos[0] = } {ypos[0] = }")
    # print(f"{np.max(xpos) = } {np.max(ypos) = }")
    # xpos_unit = xpos / np.max(xpos)
    # ypos_unit = ypos / np.max(ypos)
    xpos_unit = xpos / lcap
    ypos_unit = ypos / lcap
    Hmag = np.loadtxt(Hdir + 'Hmag.txt') * 1000  # Convert from kA/m to A/m
    # Linear interpolation of the external magnetic field - rows of Hmag correspond
...
if save_gradp:
    ...
    gradp_expr = fem.Expression(grad(p - epsilon * Gr * (h + B) + Pi(h)), W0.element.interpolation_points())
    # else:
...
p_neu_ufl = fem.form(dot(grad(p_mid + Pi(h_mid) - epsilon * Gr * (h_mid + B)), n) * ds)
...
# if add_dp == 0:
    # Qn_cap_ufl = fem.form(-1/3 * h_mid**3 * dot(grad(p_mid - epsilon * Gr * (h_mid + B)), n) * ds)
# elif add_dp == 1:
Qn_cap_ufl = fem.form(-1/3 * h_mid**3 * dot(grad(p_mid - epsilon * Gr * (h_mid + B) + Pi(h_mid)), n) * ds)
# else:
#     raise Exception(f"{add_dp = } is invalid")
...
Qn_G_ufl = fem.form(-1/3 * h_mid**3 * epsilon * Gr * dot(grad(h_mid + B), n) * ds)
Qn_bc_ufl = fem.form(Qn_cap_power * h_mid**3 * ds)

# Q_ufl = fem.form(-1/3 * h_mid**3 * (mag(grad(p_mid)) - mag_nond * M(c_mid) * mag(grad(H))) * dx)
Q_cap_ufl = fem.form(-1/3 * h_mid**3 * mag(grad(p_mid - epsilon * Gr * (h_mid + B) + Pi(h_mid))) * dx)
...
Q_G_ufl = fem.form(-1/3 * h_mid**3 * epsilon * Gr * mag(grad(h_mid + B)) * dx)
# Je_ufl = fem.form(Je * dx)

# if add_dp == 0:
#     curv_ufl = fem.form(- (p_mid - epsilon * Gr * (h_mid + B)) / pre_curv(Gamma_mid) * ds)
# elif add_dp == 1:
curv_ufl = fem.form(- (p_mid - epsilon * Gr * (h_mid + B) + Pi(h_mid)) / pre_curv(Gamma_mid) * ds)

# c flux
# Jn_c_ufl = fem.form((- 1 / Pe_c * dot(grad(c_mid), n) + c_mid * langevin(alpha * H) * dot(grad(H), n)) * ds)
Jn_c_diff_ufl = fem.form(- 1 / Pe_c * dot(grad(c_mid), n) * ds)
Jn_c_conv_ufl = fem.form(c_mid * langevin(alpha * H) * dot(grad(H), n) * ds)
...
def save_h(t_arr, h_arr):
    ...
    h_time_dir = "/home/nav/navScripts/fenicsx/ferro-thickness/paper/soap/"
    header = f"Nond time, nond average thickness for Ca = {Ca.value :.6e}, " \
             + f"Ma = {Ma.value :.6e}, Bq_s = {Bq_s.value :.6e}, Bq_d = {Bq_d.value :.6e}"
...
if rank == 0:
    ...
    save_params()
    save_h(np.array(t_t), np.array(h_domain_t))
...
if rank == 0:
    ...
    def plot_list(ax, lst, ylabel=""):
        ...
        ax.plot(np.linspace(0, 1, len(lst)), lst, 'k*-', ms=ms, label=ylabel)
    ...
    if plot:
        ...
        axs["A"].plot(np.linspace(0, 1, len(h_boundary_mean)), h_boundary_mean, 'b--')
        axs["A"].fill_between(np.linspace(0, 1, len(h_boundary_mean)), 
                              h_boundary_mean-h_boundary_std, 
                              h_boundary_mean+h_boundary_std,
                              color='red', alpha=0.3)
        ...
        axtwin = axs["B"].twinx()
        axtwin.plot(np.linspace(0, 1, len(us_n_l)), us_n_l, '*-', ms=ms, 
                    label="us_n", color="orange")
        axtwin.plot(np.linspace(0, 1, len(vs_n_l)), vs_n_l, '*-', ms=ms, 
                    label="vs_n", color="red")
        axtwin.set_ylabel(r"$\nabla Vs \cdot n$")
        axtwin.legend(loc="best")

        plot_list(axs["C"], p_neu_l, "p_neu")
        axs["C"].plot(np.linspace(0, 1, len(pmod_neu_l)), pmod_neu_l, 'b*-', ms=ms,
                      label="pmod_neu")
        axs["C"].legend(loc="best")
        axtwin = axs["C"].twinx()
        axtwin.plot(np.linspace(0, 1, len(curv_l)), curv_l, '*-', ms=ms, color="red")
        axtwin.set_ylabel(r"$\nabla^{2} h$")

        # plot_list(axs["D"], Qn_l, "Qn")
        axs["D"].plot(np.linspace(0, 1, len(Qn_cap_l)), Qn_cap_l, 'b*-.', ms=ms,
                      label="Qn_cap")
        axs["D"].plot(np.linspace(0, 1, len(Qn_mag_l)), Qn_mag_l, 'm*-', ms=ms,
                      label="Qn_mag")
        axs["D"].plot(np.linspace(0, 1, len(Qn_Pi_l)), Qn_Pi_l, 'r*-', ms=ms,
                      label="Qn_Pi")
        axs["D"].plot(np.linspace(0, 1, len(Qn_Vs_l)), Qn_Vs_l, '*-', ms=ms,
                      color='orange', label="Qn_Vs")
        axs["D"].plot(np.linspace(0, 1, len(Qn_G_l)), Qn_G_l, '*-', ms=ms,
                      color='saddlebrown', label="Qn_G")
        Qn_sum = np.array(Qn_cap_l) + np.array(Qn_mag_l) + np.array(Qn_Pi_l) \
                 + np.array(Qn_Vs_l) + np.array(Qn_G_l)
        axs["D"].plot(np.linspace(0, 1, len(Qn_sum)), Qn_sum, 'k*--', ms=ms,
                      label="Qn_sum")
        axs["D"].plot(np.linspace(0, 1, len(Qn_bc_l)), Qn_bc_l, 'c:', ms=ms,
                      label="Qn_bc")
        axs["D"].legend(loc="best")

        # plot_list(axs["E"], Q_l, "Qtotal")
        axs["E"].plot(np.linspace(0, 1, len(Q_cap_l)), Q_cap_l, 'b*-', ms=ms, label="Q_cap")
        axs["E"].plot(np.linspace(0, 1, len(Q_mag_l)), Q_mag_l, 'm*-', ms=ms, label="Q_mag")
        axs["E"].plot(np.linspace(0, 1, len(Q_Pi_l)), Q_Pi_l, 'r*-', ms=ms, label="Q_Pi")
        axs["E"].plot(np.linspace(0, 1, len(Q_Vs_l)), Q_Vs_l, '*-', ms=ms, color='orange', label="Q_Vs")
        axs["E"].plot(np.linspace(0, 1, len(Q_G_l)), Q_G_l, '*-', ms=ms, color='saddlebrown', label="Q_G")
        axs["E"].set_ylabel("|Q|")
        axs["E"].legend(loc="best")

        plot_list(axs["F"], h_domain_l, "h_domain")
        axs["F"].plot(np.linspace(0, 1, len(h_circ_l)), h_circ_l, 'g*-', ms=ms, label="h_centre")
        axs["F"].plot(np.linspace(0, 1, len(h_left_l)), h_left_l, 'b*--', ms=ms, label="h_left")
        ...
        axs["G"].plot(np.linspace(0, 1, len(Jn_c_conv_l)), Jn_c_conv_l, 'r*-', ms=ms, label="Jn_c_conv")
        axs["G"].plot(np.linspace(0, 1, len(Jn_c_diff_l)), Jn_c_diff_l, 'b*-', ms=ms, label="Jn_c_diff")
        ...
        axs["I"].plot(np.linspace(0, 1, len(c_left_l)), c_left_l, 'b*--', ms=ms, label="c_left")
        axs["I"].plot(np.linspace(0, 1, len(M_domain_l)), M_domain_l, 'g*-', ms=ms, label="M_domain")
        axs["I"].plot(np.linspace(0, 1, len(M_left_l)), M_left_l, 'r*--', ms=ms, label="M_left")
        axs["I"].legend(loc="best")

        Jn_surf_sum = np.array(Jn_surf_conv_l) + np.array(Jn_surf_diff_l)
        axs["J"].plot(np.linspace(0, 1, len(Jn_surf_sum)), Jn_surf_sum, 'k*-', ms=ms, label="Jn_sum")
        axs["J"].plot(np.linspace(0, 1, len(Jn_surf_sum)), Jn_surf_conv_l, 'r*-', ms=ms, label="Jn_conv")
        axs["J"].plot(np.linspace(0, 1, len(Jn_surf_sum)), Jn_surf_diff_l, 'b*-', ms=ms, label="Jn_diff")
        axs["J"].set_ylabel("Jn_surf")
        axs["J"].legend(loc="best")

        axs["K"].plot(np.linspace(0, 1, len(J_surf_l)), J_surf_l, 'k*-', ms=ms, label="|Jsurf|")
        axs["K"].set_ylabel("|Jsurf|")
        # axs["H"].legend(loc="best")

        axs["L"].plot(np.linspace(0, 1, len(Gamma_domain_l)), Gamma_domain_l, 'k*-', ms=ms, label="Gamma_domain")
