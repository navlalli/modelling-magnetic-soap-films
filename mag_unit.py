...
L2 = c * v2 * dx - c_n * v2 * dx \
     ...
     - dt * beta * c_mid * (1 - 6.55 * Z * c_mid) * langevin(alpha * H) * dot(grad(H), grad(v2)) * dx \
...
def import_Hmag():
    """ Import the magnetic field created by magpy """
    Hdir = main_dir + "Hfield/unit_meniscus_left/"
...
Qn_cap_ufl_left = fem.form(-1/3 * h_mid**3 * dot(grad(p_mid - epsilon * Gr * (h_mid + B) + Pi(h_mid)), n) * ds(1))
Qn_cap_ufl_right = fem.form(-1/3 * h_mid**3 * dot(grad(p_mid - epsilon * Gr * (h_mid + B) + Pi(h_mid)), n) * ds(2))
...
Qn_bc_ufl_left = fem.form(Qn_cap_power * h_mid**3 * ds(1))
Qn_bc_ufl_right = fem.form(Qn_cap_power * h_mid**3 * ds(2))
...
Jn_c_mag_ufl = fem.form(beta * c_mid * (1 - 6.55 * Z * c_mid) * langevin(alpha * H) * dot(grad(H), n) * ds)
...
def test_sol():
    ...
    Qn_bc_left = assembler(Qn_bc_ufl_left) / (boundary_area / 2)
    Qn_bc_right = assembler(Qn_bc_ufl_right) / (boundary_area / 2)
    ...
    if rank == 0:
        ...
        Qn_bc_l_left.append(Qn_bc_left)
        Qn_bc_l_right.append(Qn_bc_right)
...
if rank == 0:
    ...
    if plot:
        ...
        Qn_cap_mag = np.array(Qn_cap_l) + np.array(Qn_mag_l)
        axs["D"].plot(t_save[1:], Qn_sum, 'k*--', ms=ms, label="Qn_sum")
        axs["D"].plot(t_save[1:], Qn_cap_mag, '--', ms=ms, lw=0.75, color="forestgreen", label="Qn_cap+mag")
        ...
        Qn_right_sum = np.array(Qn_cap_l_right) + np.array(Qn_mag_l_right)
        ...
        axs["K"].plot(t_save[1:], Qn_bc_l_left, 'k*-', ms=ms, label="Qn_bc_left")
        axs["K"].plot(t_save[1:], Qn_bc_l_right, 'k*--', ms=ms, label="Qn_bc_right")
        print(f"{np.min(np.array(Qn_cap_l_left)) = }")
        axs["K"].plot(t_save[1:], Qn_left_sum, 'g*-', ms=ms, label="Qn_left_sum")
        axs["K"].plot(t_save[1:], Qn_right_sum, 'g*--', ms=ms, label="Qn_right_sum")
